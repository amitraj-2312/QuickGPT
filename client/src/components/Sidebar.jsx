...
const Sidebar = ({ isMenuOpen, setIsMenuOpen}) => {
   ...
   const deleteChat = async (params) => {
    try {
       ...
       if(data.success){
        setChats(prev => prev.filter(chat._id !== chatId))
             ...
             includes(search.toLowerCase())).map((chat)=>(
                ...
                flex justify-between group'>
                    <div>
                      ...
                      <img src={assets.bin_icon} className='hidden group-hover:block w-4 cursor not-dark:invert' alt=""  
...
<img onClick={()=> setIsMenuOpen(false)} src={assets.close_icon} className='absolute top-3 right-3 w-5 h-5 cursor-pointer md:hidden 
not-dark:invert' alt="" />
